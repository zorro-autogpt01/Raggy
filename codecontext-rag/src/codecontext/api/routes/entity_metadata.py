...
def get_entity_metadata(request: Request, repo_id: str, entity_id: str):
    ...
    # Server-side filtered lookup by ID
    entity = vector_store.get_by_id(entity_id)

    if not entity or entity.get("repo_id") != repo_id:
...
def get_file_metadata(request: Request, repo_id: str, file_path: str):
    ...
    # Get all entities in file (server-side filter)
